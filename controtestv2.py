...
def initEncoder(spi,SSEn1): 
    ...
    SSEn1.low()                #Begin SPI conversation
...
def clearEncoder(spi,SSEn1):
    ...
    SSEn1.high()          # Terminate SPI conversation
...
def readEncoder(spi,SSEn1):
    data = bytearray(4) #data 4 byte
    SSEn1.low()#Begin SPI conversation
    spi.write(bytes(0x60))  #0x60
    spi.readinto(data)  # read 4 byte
    SSEn1.high()
...
def main():
    while(True):
        ...
        initEncoder(vspi,SSEn1) #start
        clearEncoder(vspi,SSEn1)
       
        data = readEncoder(spi,SSEn1)
        data16 = int.from_bytes(ubinascii.hexlify(data), "big")
        int_val = int.from_bytes(readEncoder(vspi,SSEn1), "big")
        print('data form byte',end='')
        print(int_val)
        print('data from base 16',end='')
        print(data16)
